...
def divide_by2_handler(input_bytes):
    ...
    end_time = 1000*time()
    msg = timestamp(response, event, start_time, end_time)
    write_dict_to_state(KEY_G, msg)


def agg_handler(input_bytes):
    events = []
    keys = [KEY_B, KEY_C, KEY_E, KEY_F, KEY_G]
    for key in keys:
...
def faasm_main():
    ...
    squ_id = chain_this_with_input(square_handler, b'')
    inc_id = chain_this_with_input(inc_handler, b'')
    dou_id = chain_this_with_input(double_handler, b'')
    hal_id = chain_this_with_input(half_handler, b'')
